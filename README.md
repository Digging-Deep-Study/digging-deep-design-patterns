...
| 6 | 07/06(일), 오전 10시 | 반복자패턴 | -  | :white_check_mark: |
|   |                    | 컴포지트패턴 | -  | :white_check_mark: |
|   |                    | 상태패턴 | -  | :white_check_mark: |
| 7 | 08/03(일), 오전 10시 | 프록시패턴 | -  |  |
|   |                    | 복합패턴 | -  |  |
<!--
빈칸 양식입니다.
|   |                    | - | -  |  |
...
### 프록시(Proxy) 패턴
- 시나리오 1: AI 이미지 생성 API 접근 제한
  - 컨텍스트: 비용이 높은 외부 AI API 호출을 프록시가 캐싱과 접근 제한으로 관리
- 시나리오 2: AR 콘텐츠 접근 제어
  - 컨텍스트: GPS 위치 기반으로 AR 콘텐츠 접근 권한을 프록시가 제어

### 복합(Composite) 패턴
- 시나리오 1: IoT 스마트홈 시스템 (Observer + Command + State)
  - 컨텍스트: 장치 제어(Command), 상태 관리(State), UI 업데이트(Observer)가 복합적으로 동작
- 시나리오 2: 게임 캐릭터 행동 AI (State + Strategy + Observer)
  - 컨텍스트: AI 상태 전이, 전략 선택, 이벤트 반응이 복합적으로 결합된 행동 시스템
